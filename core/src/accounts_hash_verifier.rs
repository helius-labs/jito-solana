...
impl AccountsHashVerifier {
    ...
    fn read_epoch_accounts_hash_from_file(
        epoch: u64,
        ...
        let eah_path =
            std::env::var("SOLANA_EAH_PATH").unwrap_or_else(|_| "/home/solana/eah".to_string());
        ...
        warn!(
            "Epoch accounts hash file not found after {} attempts",
            max_attempts
        );
        None
    }
    ...
    fn process_accounts_package(
        ...
        let (accounts_hash_kind, bank_incremental_snapshot_persistence) = match accounts_package
            .package_kind
        {
            AccountsPackageKind::EpochAccountsHash => {
                if let Some(accounts_hash) = Self::read_epoch_accounts_hash_from_file(
                    current_epoch,
    ...
    fn save_epoch_accounts_hash(
        ...
        if accounts_package.package_kind == AccountsPackageKind::EpochAccountsHash {
            ...
            if !Self::snapshot_disabled(incremental_snapshot_archive_interval_slots) {
                ...
                let eah_path = std::env::var("SOLANA_EAH_PATH")
                    .unwrap_or_else(|_| "/home/solana/eah".to_string());
                ...
                match File::create(&file_path) {
                    Ok(mut file) => {
                        if let Err(e) = file.write_all(accounts_hash_string.as_bytes()) {
                            ...
                            info!("Saved epoch accounts hash to file: {}", file_path.display());
                        }
                    }
                    Err(e) => {
